...
def tf_ran_avgfilter(image, ratio, class_num = 2):
    ...
    image = tf.cond(ran<ratio, lambda: sel_filter(image, class_num = class_num), lambda: image)
    return image

def gray_color_tur(image):
    ...
    ran = tf.random_uniform([])
    image = tf.cond(ran<0.02, lambda: turn_gray(image), lambda: image)
    return image

def g_noise_tur(image, ratio=1.0):
    ...
    image = tf.cond(ran<0.1, lambda: image, lambda: add_g_noise(image, ran_std))
    return image

def tf_rotate_img(image):
    ...
    ran = tf.random_uniform([])
    image = tf.cond(ran<0.1, lambda: image, lambda: rotate_img(image))
    return image



def ran_face_size_tur_3(image, ratio=0.03):
    ...
    image = tf.cond(ran<ratio, lambda: image*generate_face_mask(img_channel), lambda: image)
    return image



def all_yuv_tur(image_in, ratio=0.03):
    ...
    ran = tf.random_uniform([])
    image = tf.cond(ran<ratio, lambda: yuv_tur_sel(image_in), lambda: image_in)
    return image

def transform_perspective(image, ratio=0.02):
    def x_y_1():
    ...
    ran = tf.random_uniform([])
    image = tf.cond(ran<ratio, lambda: trans(image), lambda:image)
    return image

def ran_img_motion_blur(image, ratio, minval, maxval):
    ...
    ran = tf.random_uniform([])
    image = tf.cond(ran<ratio, lambda:tf.cast(tf.py_func(motion_blur, [image, minval, maxval], tf.uint8), tf.float32), 
            lambda:image)
    return image

def tf_ran_resize_tur123(image, resize_ratio = 0.5, ran_ratio = 0.5):
...
def tf_ran_jpeg_compress(image, ratio):

    def inner_fun_jc(image):
        image = tf.cast(image, tf.uint8, name=None)
        image = tf.image.random_jpeg_quality(image, min_jpeg_quality = 1, max_jpeg_quality = 100)
        image = tf.reshape(image, (280, 280, 3))
        image = tf.cast(image, tf.float32, name=None)
        return image

    ran = tf.random_uniform([])
    image = tf.cond(ran<ratio, lambda:inner_fun_jc(image), lambda:image)
    return image
...
def tf_ran_rot90(image, ratio=0.03):
   ...
   ran = tf.random_uniform([])
   image = tf.cond(ran<ratio, lambda:inner_fun(image), lambda:image)
   return image

def ran_flip_up_down(image, ratio=0.02):

    ran = tf.random_uniform([])
    image = tf.cond(ran<ratio, lambda: tf.image.flip_up_down(image), lambda:image)
    return image
...
#          ran = tf.random_uniform([])
  ...
  def random_color_hue(image):
        ...
        image = tf.cond(ran_1<0.1, lambda: produce_img_1(image), lambda: image)
        return image
...
def ran_img_color_tur(image, ratio=0.2):
    ran = tf.random_uniform([])
    image = tf.cond(ran<ratio, lambda: image, lambda: img_color_tur(image, ratio=ratio, fast_mode=False))
    return image
